...
# =================== RL parameters =====================
params = RLParams(name='QLearn', alpha=0.1, epsilon=0.1, gamma=0.98, maxEpisode=100000, logPeriod=2000)
...
save_dir = fig_dir+'figures_Qlearn/'
...
def record_QL(SimulationN, Q, Q_true):
    R_num = 0
    R_sums = 0
    for _ in range(SimulationN):
        for idx in np.ndindex(roadColumn - 1, roadRow, carDirection):
            ...
            R_sum = 0
            while True:
                action = get_action(Q, s_now, 0, deadaction_flag)
...
def Alg_QL(params:RLParams, Q, SimulationN=0, Q_true=None, desc:str=None):
    alpha = params.alpha
    ...
    for td_i in td_iter:
        # Random intial state
        if td_i % logPeriod == 0:
            mes, rms = record_QL(SimulationN, Q, Q_true)
        ...
        while True:
            ...
            action_next = get_action(Q, s_next, 0, deadaction_flag)
            # Update Q-value
            ...
            if s_h_next[0] == roadColumn - 1 or s_next[0] == roadColumn - 1:
                ...
                break
            s_now = s_next
            action_now = get_action(Q, s_now, epsilon, deadaction_flag)  # Gain action, 1:left; 2:right; 3:keep
...
if __name__ == '__main__':
    ...
    Q_true = Q_true['Q']
    policy, Q, R_means, RMSE = Alg_QL(params, Q, SimulationN=2, Q_true=Q_true)
    mytime = localtime()
    ...
    X = np.arange(len(R_means)) * params.logPeriod
    print('End of RL')
    # -------------------------------------------
    # Draw optimal policy
    # -------------------------------------------
